    ...
    for tag, i1, i2, j1, j2 in sm.get_opcodes():
        if tag == "equal":
            ...
            for k in range(j1, j2):
                ...
                route = _core.Route(list(path))
                insert_op = _core.Op.insert_at(index=insert_idx, value=new[k])
                patches.append(_core.Patch(route=route, operation=insert_op))
                offset += 1
        elif tag == "delete":
...
def _apply_replace(
    ...
    for k in range(replace_count):
        ...
        if isinstance(old[i1 + k], dict) and isinstance(new[j1 + k], dict):
            child_patches = _compute_patches(old[i1 + k], new[j1 + k], child_path)
            patches.extend(child_patches)
        else:
            route = _core.Route(list(child_path))
            replace_op = _core.Op.replace(new[j1 + k])
            patches.append(_core.Patch(route=route, operation=replace_op))

    # More new items than old — insert the extras
    for k in range(replace_count, j2 - j1):
        insert_idx = i1 + replace_count + offset
        route = _core.Route(list(path))
        insert_op = _core.Op.insert_at(index=insert_idx, value=new[j1 + k])
        patches.append(_core.Patch(route=route, operation=insert_op))
        offset += 1
